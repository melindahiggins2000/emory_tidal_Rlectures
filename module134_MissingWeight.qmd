...
* [`VIM`](https://cran.r-project.org/web/packages/VIM/index.html){target="_blank"}
    - [`VIM` package website](https://statistikat.github.io/VIM/index.html){target="_blank"}
* [`skimr`](https://cran.r-project.org/web/packages/skimr/index.html){target="_blank"} - OPTIONAL
    - [`skimr` website](https://docs.ropensci.org/skimr/){target="_blank"}
* [`modelsummary`](https://cran.r-project.org/web/packages/modelsummary/){target="_blank"} - OPTIONAL
    - [`modelsummary` website](https://modelsummary.com/){target="_blank"}
* [`tinytable`](https://cran.r-project.org/web/packages/tinytable/index.html){target="_blank"} - OPTIONAL
    - [`tinytable` website](https://vincentarelbundock.github.io/tinytable/){target="_blank"}
* [`summarytools`](https://cran.r-project.org/web/packages/summarytools/){target="_blank"} - OPTIONAL
    - [`summarytools` on Github](https://github.com/dcomtois/summarytools){target="_blank"}
* [`palmerpenguins`](https://cran.r-project.org/web/packages/palmerpenguins/){target="_blank"}
    - [`palmerpenguins` website](https://allisonhorst.github.io/palmerpenguins/){target="_blank"}
* [`ggplot2`](https://cloud.r-project.org/web/packages/ggplot2/){target="_blank"}
    - [`ggplot2` website](https://ggplot2.tidyverse.org/){target="_blank"}
* [`naniar`](https://cran.r-project.org/web/packages/naniar/){target="_blank"}
    - [`naniar` website](https://naniar.njtierney.com/){target="_blank"}
* dplyr
* gtsummary

* [`mice`](https://cran.r-project.org/web/packages/mice/index.html){target="_blank"}
    - [mice website](https://amices.org/mice/){target="_blank"}
    - [BOOK: Flexible Imputation of Missing Data, 2nd ed., by Stef van Buuren (`mice` package author)](https://stefvanbuuren.name/fimd/){target="_blank"}
* 
* 
* 
...
Setting aside bias concerns for the moment, missing data logistically causes issues with code - especially in R. At first glance this seems to be a huge pain since we get errors or nonsensical output. But these issues force us to deal with the missing data and provide explicit instructions to the computer code on how we want to addressthe missing data. _Learn more in the [Flexible Imputation of Missing Data BOOK](https://stefvanbuuren.name/fimd/sec-problem.html)._
...
Table comparing `BodyWgt` and `BrainWgt` for rows with and without missing `Dream` data. Both are statistically significant - the animals with missing `Dream` valus are larger (bigger body and brain weights).
