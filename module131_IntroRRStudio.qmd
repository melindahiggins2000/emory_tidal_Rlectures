...
When you download **R** from [**CRAN**](https://cran.r-project.org/){target="_blank"} and install it on your computer, there is a R application that you can run. However, it is very bare bones. Here is a screenshot of what it looks like on my computer (Windows 11 operating system).
...
The "R" language is actually a deriviative of the [original "S" language](https://en.wikipedia.org/wiki/S_(programming_language)){target="_blank"} which stood for the "language of statistics" - it was written by statisiticans for statisticians (and now data scientists). _The original S language was written in the mid-1970's by programmers/statisticians at Bell Labs/AT&T._
...
To get an idea of how many packages may be currently on  [Github](https://github.com/){target="_blank"}, we can "search" for "R package" [https://github.com/search?q=R+package&type=repositories](https://github.com/search?q=R+package&type=repositories){target="_blank"} and as you can see this is well over 118,000+ packages.

![](github_rpkg.png)

While you can find packages on Github that have not (yet) been published on CRAN or Bioconductor, the developers of packages currently on CRAN and Bioconductor also often publish their development version (think of these as in "beta" and still undergoing testing) on Github. For example, the current published version of the data wrangling R package [`dplyr` on CRAN](https://cran.r-project.org/web/packages/dplyr/index.html){target="_blank"} was last updated on 11/17/2023.

![](cran_dplyr.png)

But the development version of [`dplyr` on Github](https://github.com/tidyverse/dplyr){target="_blank"} was last updated 5 months ago in August 2024. _There is probably a new version of `dplyr` coming soon for CRAN._
...
While it may seem worrisome that there is no governing company or organization that verifies and validates and certifies all R packages, the good news is that the R community is a vast Global community. The development of R is not controlled by a limited number of people hired within some single company - instead there are literally millions of R programmers across the Globe testing and providing feedback on a 24/7 basis. If there is a problem with a package or function, there will be people posting about these issues - see [Additional Resources](additionalResources.html#other-places-to-get-help){target="_blank"}. 

**This is the power of Open Source computing!!**
:::

To get an idea of how long a package has been in use and if it is still being actively supported and how it relates to other similar packages, check out this interactive Shiny app website for [CRAN downloads](https://hadley.shinyapps.io/cran-downloads/){target="_blank"}. Type in the packages you want to compare and change the dates.
