...
* Optional [`gapminder`](https://cran.r-project.org/web/packages/gapminder/)
* Optional [`gganimate`](https://cran.r-project.org/web/packages/gganimate/)
* Optional [`plotly`](https://cran.r-project.org/web/packages/plotly/)
* Optional [`gt`](https://cran.r-project.org/web/packages/gt/)
* Optional [`gtExtras`](https://cran.r-project.org/web/packages/gtExtras/)
...
Notice that by adding `color = GenderCoded.f` inside the `aes()` aesthetic that a legend for the coloring of the points is automatically added to the plot. This can be disabled if you wish. Learn more about colors and legends in the [`ggplot2` book - Chapter 11](https://ggplot2-book.org/scales-colour).
:::
...
* assigned custom shapes for each gender category,
    - the colors are for the non-missing values
    - if you want to see the person missing a gender, we have to specifically assign a color for NA using `na.value=`
* also notice that I had to provide a custom label in the `labs()` for the shape and color legend - **the labels are the same for `color` and `shape`** so they will be in the same legend box. _It is possible to assign the variables for color and shape to different variables._
...
First let's compute the means using the `mean()` function and we'll pull the 95% confidence interval limits from the `t.test()` output for a one-sample t-test _(see details below)_.
...
If we ask to "see" the `tt1` object printed to the console, we get the abbreviated t-test results:

```{r}
tt1
```

But the `tt1` t-test output object actually has a bunch of details stored inside it. Let's look at the structure of the t-test object:
...
So, we use this knowledge inside the `mutate` step in the code below to make sure we capture the lower confidence interval `lci` and upper confidence interval `uci` for each gender category which we will later use for making our errorbars.
