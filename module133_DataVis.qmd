...
There is one person missing SES. So, let's filter the dataset and remake the plot. Instead of creating another "new" dataset, we can instead using the `dplyr` pipe `%>%` into our plotting workflow as follows to filter out the missing SES before we make the plot. Notice I can drop the `data = ` in the `ggplot()` step.
...
We can also add points on top of the boxplots using `geom_jitter()` add AFTER `geom_boxplot`. If you switch the order of these "geom's" you can specify whether the boxplot is on top of the point or if the points are in top of the boxplots (like we did here). 
...
Here is an interactive version (on this website for the HTML version) of the side-by-side boxplots above - with a horizontal orientation. _This plot will NOT be interactive in the PDF document but the figure will be shown._
